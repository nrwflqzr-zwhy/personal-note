...
![img](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70.png)
...
int index(String main_str, String aim_str){
    int i = 0;
	int j = 0; // i 和 j 分别用来扫描主串和目标串
    int k = 0;
	while(i <= main_str.size() - 1 && j <= aim_str.size() - 1){
		if (main_str.charAt(i) == main_str.charAt(j){
			++i;
			++j;
		}else{
			j = 1;
...
1.   暴力匹配的缺点
     ...
     ![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-17194761420903.png)
...
2.   最长相同前缀和后缀
     ...
     在继续讲述之前，必须引入一个概念——一个字符串的**最长相同前缀和后缀**。要想知道什么是最长相同前缀和后缀，首先得明白什么是字符串的前缀和后缀，相信看完下面这个图你就不难理解了

     ![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-17194762365706.png)

     那么，最长相同前缀和后缀你也应该能找出来了吧

![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-17194799445269.png)
...
3.   究竟怎么回溯
     ...
     ![image-20240627171942579](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/image-20240627171942579.png)

     不仅仅是这样，继续观察，目标串发生不匹配时其索引为 5，索引 5 之前的字符串为 “ABCAB”，其最长相同前后缀长度为 2，同时目标串中也正好是索引为 2 的地方（也即字母C）与主串发生不匹配的地方（也即字母E)对齐

     这意味着所谓让最长相同前缀对齐至后缀其本质就是：记**目标串发生不匹配的地方前**的字符串的**最长相同前后缀长度为 l** ，然后回溯时让目标串索引为 l 的地方对齐至此次不匹配的地方。（这里的理解是，l 的位置是跳过最长相同前缀的位置，将 l 与不匹配的位置对齐正好让最长相同前缀和最长相同后缀对齐——因为之前已经匹配过了）
...
4.   next数组
     ...
     ![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/20210323210510280.png)
     ...
     ![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948038231214.png)

### 求解next数组

KMP算法的思想大部分人是能很轻松的明白的，但是这个算法为什么劝退了很多人呢？就是无法准确，深刻的理解求解 next 数组的代码，如下

上述代码中总共有三个地方如果能理解清楚这个代码也就没问题了。 next[0] = -1, next[j] = l, k = next[k]，其中 k = next[k] 最让人摸不着头脑。

![image-20240627181417237](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/image-20240627181417237.png)

```java
void getnext(String target_str, int next[]){
    int j = 0, k = -1; // k == -1 说明上一个位置判断最长相同前缀后缀时失败，因此需要重新判断最长相同前缀后缀
    if (j < target_str.length){
        if(k == -1 || target_str[k] == target_str[j]){ // k != -1 时，表明判断上一个 j 位置时它们已经匹配了最长相同前缀后缀，那么此时 k 位置和 j 位置又相同了，则最长相同前缀后缀又要加长长度 1
            j++;
            k++;
            next[j] = k;
        }else{
            k = next[k]; // 解释：可以假设 str[k] == str[j]（图中的C改为B） 那么上图的 j = 8 的位置如果匹配失败需要回溯到 k = 3 的位置（最长的前缀是 ABA），但是现在 str[k] !=str[j] 所以即使回溯到 k = 3 的位置也同样是大概率失败的匹配（不排除恰好 k = 3 的地方是 E），既然如此跳过这次大概率失败的匹配，令 k = next[k]（因为 k < j，所以 k 要回溯的位置是确定的）
        }
    }
}
```
...
void getnext(string target_str, int next[]){
	...
	while(j < target_str.size()){
		if (k == -1 || str[j] == str[k]){
			j++; //这里++ 过后，判断的都是下一个位置的 next 值，因此直接开始判断第二个的值
			k++;
			next[j] = k;
		}else{
			k = next[k];
...
如下，next[0]=-1是指当目标串的第一个位置发生不匹配时，应该让目标串的索引为 -1 的位置与当前不匹配的地方对齐

你肯定会问，怎么可能有-1的位置啊，这明显越界了啊？

![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948119007317.png)

这里我们先截取KMP算法的一小段（不要担心你肯定看得懂）

下面的 j=next[j] 指的就是发生不匹配时应该如何回溯

![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/20210323213527766.png)



因此当代码运行到这里时就会出现 j= next[0] = -1，然后重新回到循环，继续判断，此时 j=-1，进入 if 内，j++很明显会等于0，i++会到主串的下一位。所以它的意思就是让目标串的第一位与主串的下一位进行比较

现在你应该能理解为什么要设置next[0]=-1了吧

#### B：next[j] = k

对于next[j]=k我认为大家肯定能明白的一点就是在为next数组进行赋值，但是不明白的是为什么下标为 j 的元素的值是 k
...
![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948143145522.png)

非常幸运，他们相同，于是j++，k++，next[j]=3

![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948147596325.png)
...
![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948148765628.png)

很遗憾这个位置，无法使其成为长度为4的相同前后缀，那么根据代码只能执行 k=next[k] 了。这样画比较难受，我把前缀直接拿到下面来

![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948150607231.png)
...
![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948158214734.png)

好的，重新组装回去，毕竟这不是真的主串和目标串

![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948158936437.png)
...
![在这里插入图片描述](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MTgzMDM0,size_16,color_FFFFFF,t_70-171948160771540.png)
...
[最大化一张图中的路径价值](#2065、最大化一张图中的路径价值)
...
![image-20240102201450144](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/image-20240102201450144.png)
...
class Solution {
    public int paintWalls(int[] cost, int[] time) {
        int n = cost.length; //最多需要这么多油漆匠
        int[] f = new int[n * 2 + 1]; // 保存初始状态
        Arrays.fill(f, Integer.MAX_VALUE / 2);
        f[n] = 0; // 这里对应题解中的 f(0, 0) = 0，因为我们对 j 进行了范围处理
        for (int i = 0; i < n; ++i) {
            int[] g = new int[n * 2 + 1];
            Arrays.fill(g, Integer.MAX_VALUE / 2); // 如果 Integer.MAX_VALUE 的话后面加 cost[i] 会溢出
            for (int j = 0; j <= n * 2; ++j) {
                // 付费
                g[Math.min(j + time[i], n * 2)] = Math.min(g[Math.min(j + time[i], n * 2)], f[j] + cost[i]); // f 是上一次循环的状态（只考虑到 0 ~ i - 1 位油漆匠，而现在在考虑第 i 位油漆匠
                // 免费
                if (j > 0) {
                    g[j - 1] = Math.min(g[j - 1], f[j]); // 题解中 i 在这种实现中是隐藏的
                }
            }
            f = g;
        }
        int ans = f[n];
        for (int i = n + 1; i <= n * 2; i++) { // 负的那半不用管，因为不合理
...
![img](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/ex1drawio.png)
...
![img](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/ex2drawio.png)
...
![img](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/ex31drawio.png)
...
**![img](%E5%8A%9B%E6%89%A3%E9%A2%98%E8%A7%A3.images/ex4drawio.png)**
...
class Solution {
    ...
    int[] values;
    int maxTime;
    List<int[]>[] g; // g 是一个 List<int[]> 类型的数组
    boolean[] visited;
    ...
    public int maximalPathQuality(int[] values, int[][] edges, int maxTime) {
        ...
        for (int[] edge : edges) {
            g[edge[0]].add(new int[]{edge[1], edge[2]}); // 储存它的临边和价值，这种方式比二维数组好
            g[edge[1]].add(new int[]{edge[0], edge[2]});
        }

        visited = new boolean[n]; // boolean 的默认值是 false
        visited[0] = true; // 节点 0 访问过置为 true
        
        dfs(0, 0, values[0]); // 深度优先搜索
        return ans;
    }

    public void dfs(int u, int time, int value) {
        if (u == 0) {// 重新回到 0 之后，判断最大的价值
            ans = Math.max(ans, value);
        }
        for (int[] arr : g[u]) {
            int v = arr[0], dist = arr[1];
            if (time + dist <= maxTime) { // 距离足够
                if (!visited[v]) { // 结点还未被访问过，因为每个结点价值在路径中只算一次
                    visited[v] = true;
                    dfs(v, time + dist, value + values[v]);// 之前没有访问过，走这条路径
                    visited[v] = false; // 走其他路径
                } else {
                    dfs(v, time + dist, value);// 之前已经访问过，但是还是走这条路径
                }
            }
        }
    }
